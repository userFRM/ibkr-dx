...
Every request reads its number as a gateway reads it, four bytes signed: one
that does not fit — every id at and above `0x8000_0000`, the ones this client
keeps at and above `0xC000_0000` for the questions it asks itself among them —
is refused under -1 with 320, *Error reading request: Unable to parse field:
'Client Req Id' for input string: '…'*, as a gateway refuses it, whatever else
the request states. A withdrawal refused so withdraws nothing; a stream `watch`
opened is withdrawn under the number it handed back. A negative id is carried
whole by a market-data request, as a gateway reads it, and refused by the
requests that carry the number as four bytes unsigned.
The interface this client mirrors
encourages one counter for orders and requests, and a program that keeps one
meets this once the account's order ids reach `0x8000_0000`: the next valid id
