...
impl EClient {
    ...
    /// clock boundary a caller expects. A `1 day` bar is the one the history
    /// last stated until that ends; the next five-second bar then opens a bar
    /// at midnight UTC of its own day, as a gateway opens it, ending at the
    /// next midnight UTC or at the close the history stated where that falls
    /// between the two, and a bar that has ended is opened again by the next
    /// five-second bar. Each is dated by where it ends, on the series' zone. A
    /// week opens on its Monday and a month on its first day, both at midnight
    /// UTC, on the calendar as a gateway folds them.
